...
router.post('/answer/:uid', csrfProtection, function (req, res, next) {
    ...
    firebaseAdmin_DB.ref('users/' + uid).once('value').then(snapshot => {
        ...
        var mailOptions = {
            ...
            to : user_email,
            subject : '書裡回覆你的問題了',
            html: `<p>${full_answer}你的序號為：${Serial_number} <br> 請點擊：<a href="https://dashboard.heroku.com/apps/users/confirm/${response_key}">確認預約本書</a>，在${reserved_deadline_email}前來，開啟這趟自我探索的旅途。</p>`
        };
